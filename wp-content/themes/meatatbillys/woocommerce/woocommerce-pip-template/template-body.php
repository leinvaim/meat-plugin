						...
						<?php if (get_post_meta( $order_id, '_wcms_packages', true )) { ?>
							<?php $packages = get_post_meta( $order_id, '_wcms_packages', true );
								foreach ($packages as $package):
									echo '<p>' . WC_Pip_Compatibility::WC()->countries->get_formatted_address( $package['full_address'] ) . '</p>';
								endforeach;
							?>
						<?php }
							else { ?>
			...
			<table>
				...
				<tfoot>
					...
					<th colspan="2" style="text-align:left; padding-top: 12px;">&nbsp;</th>
						...
						<td style="text-align:left;"><?php echo woocommerce_price( $tax_item->amount ); ?></td>
					</tr>
					<?php endforeach; ?><tr>
						<th colspan="2" style="text-align:left; padding-top: 12px;">&nbsp;</th>
